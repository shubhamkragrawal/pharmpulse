...
with sponsor_first_year as (
    ...
    from {{ ref('dim_sponsor') }} as dim_sponsor
    inner join {{ ref('fct_trials') }} as fct_trials using (sponsor_key)
...
sponsor_trial_activity_years as (
    ...
    from {{ ref('fct_trials') }} as fct_trials
    where fct_trials.start_date is not null
...
final as (
    ...
    select
        {{ generate_surrogate_key(['cohort_activity.cohort_year', 'cohort_activity.activity_year']) }} as cohort_activity_key,
